    ...
    def on_wrong():
        ...
        def on_save():
            ...
            if val == "":
                ...
                messagebox.showwarning("E3 name is invalid", "E3 name is not matching any database entry. \n "
                                                                                     "Please insert correct E3 name.")
                return
            elif df["Text"].eq(test_step).any():
                messagebox.showwarning("Test step already included", "The test step is already included in the training database. \n "
                                                                     "Proceed without adding text to training database.")
                feedback["status"] = 'incorrect'
                feedback["correct_label"] = val
                dialog.destroy()

            feedback["status"] = 'incorrect_excel'
            feedback["correct_label"] = val
            dialog.destroy()
